...
| 3D 地理 | `kanyu_scene3d` | 挤出体场景数据制备（高度字段拉伸 + colorField 分类着色 ≤12 类 + heightRange 高度范围摘要；回执附工作台 3D 页签接力指引）+ Client canvas 软件 3D 管线（对齐 scene3d.rs：yaw/pitch 拖拽旋转/背面剔除/质心纵深排序/两档明暗/类别色+图例） |
