...
| GIS 数据目录读取 | `kanyu_catalog` / `kanyu_data` | 五分类工程目录（壳层 catalog.rs 范式）+ WFS 服务链接发现/拉取（services.discover/fetch）+ `kanyu data info/query/validate` + `data.preview` 属性表（纯 fs）+ 格式注册表 |
