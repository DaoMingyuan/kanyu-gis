...
- **面板侧 20 RPC**（工作台页签 ↔ Host 半；静态形态经 `/kanyu-gis/call` 桥）：`ping` / `introspect` / `catalog.list` / `services.discover` / `services.fetch` / `data.info` / `data.query` / `data.validate` / `data.preview` / `render.map` / `crs.presets` / `crs.reproject` / `geoprocess.list` / `geoprocess.run` / `edit.ops` / `edit.apply` / `edit.undo` / `edit.redo` / `edit.history` / `scene3d.data`。
