...
- **底部会话/终端融合 + 地图白底（第一百轮）**：壳内底部条 `.kyg-bottombar`
  三页签——「地图」满高 /「会话」压壳 52% 露出宿主会话区（会话功能完整性
  由宿主保证）/「终端」壳内受限控制台（`TabTerm` + 新 RPC `console.run`：
  kanyu 子命令白名单直通，拒绝链式/重定向/替换符号，无任意代码执行安全
  基线，RPC 面 34→35）；顶栏「返回会话」钮删除，「收起」入底部条
  （`kyg-reopen` 浮动钮保留）。地图/编辑画布白底 `#ffffff`（对齐 ArcGIS
  Pro 纯白画布），壳底色向 DSH 中性灰对齐（`#16181d`，topbar 中性化）。
  测试器 269/269（static 212/212，+3），agent-browser 3080 实测：白底
  渲染 / 终端 `data info` 出图层信息 / 非白名单命令拒绝 / 会话压壳露
  composer + 截图目检。

## [0.94.0] — 2026-08-19

- **分析工具右侧化（第九十九轮）**：地理处理按 ArcGIS Pro 工具箱范式迁入
  右侧停靠面板——新增 `.kyg-right`（300px，border-left 分隔，独立滚动）；
  ribbon 「处理」页签撤下（`hidden: true`），「分析处理」组内加「分析」
  开关钮（`gpOpen` state）；`TabGp` 头部加工具搜索框（`gpq`，名称/id
  子串过滤）+ 精选工具清单行（`kyg-list-item` + GP 徽标，点击即选），
  参数表单与工具箱全库注册表保留。测试器 266/266（static 209/209，+2），
  agent-browser 3080 实测：搜索「缓冲」过滤命中、选 buffer 填参运行
  输出 4 要素 GeoJSON、截图目检配色与宿主一致。
...
- **3D 高程夸张系数（第九十五轮 ArcGIS 垂直夸张语义）**：`drawScene3d`
  新增 `exag` 形参——高度归一化 zScale（最大高度锚定画布 1/4 高，内核
  height_scale 同式）乘夸张系数，纯显示乘算不改数据；Tab3d 加「夸张」
  下拉（×0.5/1/1.5/2/3/5），画布标注行显示当前档位。测试器 253/253
  （static 196/196），agent-browser 3080 实测 poly3d 双棱柱 ×3 高耸
  顶格 vs ×0.5 低矮体块、标注同步「夸张 ×N」。

## [0.89.0] — 2026-08-19

- **identify 浮层「定位至此」（第九十四轮）**：`data.identify` 回执新增
  `centroid`（几何范围中心，顶点递归 bbox 中点近似）；浮层头加「定位」
  钮——一键 zf=2 缩放到命中要素并按 `pan = (容器中心 − 要素内容点)×倍率`
  反解居中（translate+scale origin=center 的逆变换），比例尺随缩放档
  实时减半。测试器 251/251（static 194/194），agent-browser 3080 实测
  定位示例大厦A 后要素距视口中心 +2/−1 px、缩放 ×2.00、比例尺
  1:8,025→1:4,013。
