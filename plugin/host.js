  ...
  apply(ctx) {
    ...
    textTool({
      ...
      parameters: {
        ...
        param: { type: 'object', additionalProperties: true, description: '技能参数键值（buffer_zones 必填 {"_distance": 数值}——缓冲距离，地图单位、> 0；overlay_ops 必填 {"_op": "intersect/union/difference"}；dissolve_field 必填 {"_field": "字段名"}——融合分组字段；stat_summary 必填 {"_stat": "数值字段名"}、可选 {"_field": "分组字段名"}；注入 _role=param 参数要素走临时输入）' },
