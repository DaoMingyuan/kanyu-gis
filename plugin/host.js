...
return {
  ...
  apply(ctx) {
    ...
    textTool({
      name: 'kanyu_skill',
      description: 'WASM 技能沙箱（总规 §4.5「以 WASM 为技能」，kanyu skill run 出口）：内置 split_polygons 面切割——cutLine 切割线横贯面要素劈分（geo Buffer+BooleanOps 差集，属性继承 + _part 序号；切割线须横贯目标面，原数据不动、产出新图层）、buffer_zones 缓冲区——param {_distance} 按距离膨胀点/线/面为面（geo Buffer round join，属性继承 + _distance/_part；距离须为正地图单位）、overlay_ops 叠加分析——param {_op: intersect/union/difference/clip} + input2 第二图层（geo BooleanOps：相交两两配对基准属性继承 / 合并整体 / 差集基准减叠加 / 裁剪 ArcGIS Clip 语义=基准面整体∩叠加整体一次性交集不两两配对、叠加属性不入产出，仅面要素）、dissolve_field 融合——param {_field} 按字段分组合并面要素（geo BooleanOps union 组内折叠，properties 留分组字段 + _count，多部附 _part）、stat_summary 统计——param {_stat 必填数值字段, _field 可选分组字段} 纯属性聚合（无几何输出，每组一要素带 _count/_skipped/_sum/_min/_max/_avg；宿主类型化列把混合列强制为字符串时兼容解析数值字符串，真正非数值跳过计 _skipped）、simplify_geom 几何简化——param {_tolerance} RDP 容差抽稀线/面顶点（geo Simplify，属性继承 + _tolerance/_verts 前后顶点数；容差须为正地图单位，点系透传、退化跳过）、measure_geom 几何量算——param {_measure: area/length} 逐要素量算写 _area/_length（shoelace 外环减内环 / 欧氏长度；类型不匹配透传计 _skipped 语义，平面坐标——经纬度请先投影）；技能为 wasmtime 沙箱 WASM 组件（无 IO、fuel 配额），单一事实源 guest 源码在 dsh/skills/。',
      parameters: {
        skill: { type: 'string', required: true, description: '技能路径（内置：dsh/skills/split_polygons.wasm / dsh/skills/buffer_zones.wasm / dsh/skills/overlay_ops.wasm / dsh/skills/dissolve_field.wasm / dsh/skills/stat_summary.wasm / dsh/skills/simplify_geom.wasm / dsh/skills/measure_geom.wasm）' },
        ...
        param: { type: 'object', additionalProperties: true, description: '技能参数键值（buffer_zones 必填 {"_distance": 数值}——缓冲距离，地图单位、> 0；overlay_ops 必填 {"_op": "intersect/union/difference/clip"}——clip 为 ArcGIS Clip 裁剪语义：基准面整体 ∩ 叠加整体一次性交集、不两两配对、叠加属性不入产出；dissolve_field 必填 {"_field": "字段名"}——融合分组字段；stat_summary 必填 {"_stat": "数值字段名"}、可选 {"_field": "分组字段名"}；simplify_geom 必填 {"_tolerance": 数值}——RDP 简化容差，地图单位、> 0，点系透传；measure_geom 必填 {"_measure": "area/length"}——逐要素量算写 _area/_length，类型不匹配透传；注入 _role=param 参数要素走临时输入）' },
    ...
    console.log('kanyu-gis Host 半已激活：7 大能力 RPC + 9 动态工具，工作区 ' + WORKSPACE)
  },
}
