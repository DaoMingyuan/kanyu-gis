...
function ToolboxPanel(props) {
  ...
  async function tbRun() {
    ...
    try {
      const r = await host.call('toolbox.run', { id: tbId, params: tbKv, output: tbOutPath || undefined })
...
function Tab3d(props) {
  ...
  async function load() {
    ...
    try {
      const r = await host.call('scene3d.data', { path, heightField: hf, maxFeatures: 300, colorField: cf || undefined })
