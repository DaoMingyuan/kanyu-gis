...
function TabMap(props) {
  ...
  async function onMapIdentify(e) {
    ...
    try {
      const res = await host.call('data.identify', { path: store.path || path, x: mx, y: my, tol })
  ...
  async function loadBasemap(p, w, hh) {
    try {
      const ir = await host.call('data.info', { path: p })
      const j = JSON.parse((ir && ir.stdout) || '{}')
      if (!j.extent) { setBaseImg(null); return }
      const br = await host.call('services.wms', { url: baseUrl, layer: baseLayer, bbox: j.extent, width: w, height: hh, axisSwap: true })
...
function TabTerm({ store, notify }) {
  ...
  async function run() {
    ...
    setBusy(true)
    const r = await host.call('console.run', { line: cmd })
...
function drawScene3d(cv, data, view, exag) {
  ...
  const cx = W / 2, cy = H / 2
  // 高度归一化（内核 height_scale：最大高度锚定画布 1/4 高）× 高程夸张
  // 系数（第九十五轮 ArcGIS 垂直夸张语义：纯显示乘算，不改数据）
  let maxH = 0
...
return {
  ...
  apply(ctx) {
    ...
    function Workbench() {
      ...
      React.useEffect(() => {
        host.call('catalog.list', { dir: s.scanDir || undefined, depth: 3 }).then(r => {
      ...
      async function pickProject(kyu) {
        ...
        try {
          const r = await host.call('style.list', { kyu })
