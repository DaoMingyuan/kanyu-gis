...
`
    ...
    function TabCatalog(props) {
      ...
      async function previewLayout(it) {
        ...
        try {
          const r = await hostCall('render.layout', { kyu: it.kyu, title: it.title })
      ...
      async function previewMapImage(it) {
        ...
        try {
          const r = await hostCall('catalog.readImage', { path: it.path })
    ...
    function TabData(props) {
      ...
      async function runQuery() {
        ...
        try {
          ...
          // 预览不可达时降级仅保留计数回执
          try {
            const pv2 = await hostCall('data.preview', { path: outPath, limit: 50 })
    ...
    // 地图：离屏渲染面板
    // 符号化编辑模型 LayerSymbology 直通 render.map symbology（Host 半
    // symToRule 投影为 StyleRule——对齐壳层 symbology.rs/.kyu 持久化格式，
    // 第五十二轮由裸 StyleRule 文本切换为编辑模型）。
    // categorical 文本「类别:#RRGGBB,…」；graduated 断点文本「阈值,…」严格升序。
    ...
    function TabMap(props) {
      ...
      async function render2d(p) {
        ...
        try {
          const r = await hostCall('render.map', { path: usePath, theme: 'light', width: w, height: Math.round(w * 0.62), symbology: sym, transparent: baseOn })
      ...
      async function styleLoad() {
        setBusy(true)
        try {
          const r = await hostCall('style.get', { kyu: kyuPath, layerId })
          if (r && r.ok) {
      ...
      async function styleSave() {
        ...
        setBusy(true)
        try {
          const r = await hostCall('style.set', { kyu: kyuPath, layerId, style: sym })
...
h('div', { className: 'kyg-hint' }, msg),
    ...
    function TabCrs(props) {
      ...
      async function search() {
        setBusy(true)
        try {
          const r = await hostCall('crs.search', { query, limit: 20 })
      ...
      return h('div', null,
        ...
        hits ? h('div', null,
          h('div', { className: 'kyg-hint' }, hits.length + ' 条结果 · ' + searchSrc + '（点「源」/「目标」按钮分设 CRS）'),
          hits.map(c => h('div', { key: c.code, className: 'kyg-row' },
            h('span', { style: { fontFamily: 'monospace', marginRight: '6px' } }, c.code),
            h('span', { style: { flex: 1 } }, c.name),
            h('span', { className: 'kyg-hint' }, c.kind + (c.unit ? ' · ' + c.unit : '')),
            h('button', { className: 'kyg-btn kyg-btn-sub', style: { padding: '1px 8px', fontSize: '11px' }, onClick: () => setFrom(c.code) }, '源'),
            h('button', { className: 'kyg-btn kyg-btn-sub', style: { padding: '1px 8px', fontSize: '11px' }, onClick: () => setTo(c.code) }, '目标')))) : null,
        Field('数据', h('input', { className: 'kyg-input', value: path, onChange: e => setPath(e.target.value) })),
    ...
    // 2D 画布绘制：drag.batch 存在时选择集内其余顶点按同一像素位移联动预览
    // （框选批量移动）；opts = {rect, sel} 时叠加框选橡皮筋 + 选中顶点高亮
    ...
    function TabEdit(props) {
      ...
      async function vUp() {
        ...
        try {
          ...
          if (d.batch) {
            ...
            r = await hostCall('edit.apply', { path, op: 'vertices-move', args: { moves }, inPlace })
          } else {
            setOut('写入顶点 (' + rx + ', ' + ry + ')…')
            ...
            r = topoMode
              ? await hostCall('edit.apply', { path, op: 'topo-move',
                  args: { x: d.sel.pos[0], y: d.sel.pos[1], nx: rx, ny: ry }, inPlace })
              : await hostCall('edit.apply', { path, op: 'vertex-move',
      ...
      async function doAddPoint(rx, ry) {
        ...
        try {
          const r = await hostCall('edit.apply', { path, op: 'feature-add',
      ...
      async function applyDrawNew() {
        ...
        try {
          const r = await hostCall('edit.apply', { path, op: 'feature-add', args: { geometry }, inPlace })
          setOut(fmtJson(r))
      ...
      async function applyCutPoly() {
        ...
        try {
          const r = await hostCall('skill.run', { skill: 'dsh/skills/split_polygons.wasm', input: path, output: outPath, cutLine: pts })
          if (r && r.ok) {
      ...
      async function applyMeasure() {
        ...
        try {
          const r = await hostCall('skill.run', { skill: 'dsh/skills/measure_geom.wasm', input: path, output: outPath, param: { _measure: meaOp } })
      ...
      return h('div', null,
        ...
        h('div', { className: 'kyg-row' },
          h('select', { className: 'kyg-input', style: { maxWidth: '26%' }, value: ovlOp, onChange: e => setOvlOp(e.target.value) },
            ...
            h('option', { value: 'clip' }, '裁剪 clip（叠加层作模子）')),
    ...
    function Tab3d(props) {
      ...
      async function load(p) {
        ...
        try {
          const r = await hostCall('scene3d.data', { path: usePath, heightField: hf, maxFeatures: 300, colorField: cf || undefined, symbology: sym || undefined })
    ...
    exports.apply = function apply(ctx) {
      ...
      // ------ 全屏接管：工作台铺满会话中央列 ------
      // shell.overlay 层 inset:0 覆盖整个应用框（pointer-events:none，子元素
      // auto）；中央列矩形由 [class*=centerCol] getBoundingClientRect 同步
      // （ResizeObserver + window resize），position:fixed 落位——侧栏/详情列
      // 保持原生可交互，结构上等价参考形态的「地理工作台」中央接管。
      ...
      // 当前图层变为清单外新文件（查询/编辑/服务产出）时自动重扫一次
      function Dock(props) {
        const store = props.store
        ...
        async function toggleVis(l, vis) {
          const r = await hostCall('style.setVisible', { kyu: store.kyuProject.kyu, layerId: l.id, visible: vis })
        ...
        async function scan() {
          setMsg('扫描中…')
          try {
            const r = await hostCall('catalog.list', { depth: 3 })
      ...
      function Workbench() {
        ...
        if (!gis) return null
        // 手动「返回会话」后留悬浮重开钮（首页/新会话视图无会话头部槽位，
        // 没有它用户将无法自行回到工作台）
      ...
      console.log('kanyu-gis Client 半已激活：会话头部按钮 + 全屏工作台（随 kanyu-gis preset 联动接管会话中央列）')
    }
